...
function HostPayments() {
  ...
  const [deposit, setDeposit] = useState({ total_amount: "", organizer_percentage: 70, gigs_percentage: 30, payment_method: "upi", upi_transaction_id: "" });
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const [walletRes, eventsRes, poolsRes] = await Promise.all([
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg text-sm hover:shadow"
                >
        ...
        <section className="bg-white rounded-2xl shadow-lg p-6">
          ...
          {!escrow ? (
            ...
            <div className="border rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-gray-900">Status: <span className="capitalize">{escrow.status}</span></p>
                  <p className="text-sm text-gray-600">Organizer: {escrow?.organizer?.email || ""}</p>
                </div>
                <button
                  onClick={onRelease}
                  disabled={busy.release || escrow.status === "released"}
                  className={`px-3 py-2 text-sm rounded-lg flex items-center space-x-2 ${escrow.status === "released" ? "bg-green-100 text-green-700" : "bg-indigo-600 text-white"}`}
                >
                  <FaCheckCircle />
